...
@export var sword_offset2: Vector2 = Vector2(-95, 28)
@export var wand_offset: Vector2 = Vector2(35, 28)
@export var wand_offset2: Vector2 = Vector2(-35, 28)
...
func _ready()->void:
	player = GlobalData.get_player1_character()
	GlobalData.save_player1_weapon(weapon)
#Here we checked if they have done the doulbe jump or not, and if the jump is possible it counts it, starts the timer so that they don't doulbe jump by mistake.
func jump():
...
#Gets direction to attack based on last pressed button
...
func shoot_bullet():
	#Instantiate a new bullet instance from the preloaded bullet scene and casts it as a RigidBody2D
	var bullet = bullet_scene.instantiate() as RigidBody2D
	#Gets direction
	var direction = get_weapon_direction()
	#Sets the bullet's position just ahead of the character's current global position to avoid hitbox collision
	bullet.position = global_position + shoot_offset * direction
	#Align the bullet's rotation with the character's rotation to match its firing direction
	bullet.rotation = direction.angle()
	#Adds the bullet to the scene as a child of the character's parent
	get_parent().add_child(bullet)


func get_weapon_offset(base_offset: Vector2) -> Vector2:
	return Vector2(base_offset.x * facing_direction, base_offset.y)
...
func sword_appear():
	if sword_instance == null:
		...
		var adjusted_offset = sword_offset
		if facing_direction == -1:
			adjusted_offset.x = -sword_offset.x
		sword_instance.position = global_position + adjusted_offset
		sword_instance.scale.x = facing_direction
...
func start_wand_charge():
	if staff_instance == null:
		# Create the staff without rotation
		staff_instance = wand.instantiate() as Node2D
		var direction = get_weapon_direction()
		staff_instance.position = global_position + wand_offset * direction
...
func fire_wand_projectile():
	if is_charging_wand:
		...
		# Correctly position and set the direction
		projectile.position = global_position + shoot_offset * direction
		projectile.call("set_direction", facing_direction)
...
func _physics_process(delta):
	...
	if staff_instance != null:
		var wand_pos = wand_offset
		if facing_direction == -1:
			wand_pos.x = -wand_offset.x
		staff_instance.position = global_position + wand_pos
