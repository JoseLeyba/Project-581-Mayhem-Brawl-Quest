...
Purpose: Chicken Enemy Script
'''

extends CharacterBody2D

#The gravity for falling physics, not applicable as draagon floats
var gravity = 0.0
#The speed of the enemy
var SPEED = 200
var dir = 1
#These variables are related to the Dragon flying
var time_passed = 0.0
var amplitude = 70.0
var frequency = 2.0 
var initial_y = 0.0
...
@export var feather_scene = preload("res://Scenes/Enemies/featherBoss.tscn")
@onready var enemy_sprite = $AnimatedSprite2D
#Dying sound
@onready var die_sound: AudioStreamPlayer2D = $AudioStreamPlayer2D  

@onready var ray_cast_up = $up
...
#This function will kill the draagon
func die(playerID):
...
func _physics_process(delta):
	...
	#The dragon will float up and down depending on the time passed
	time_passed += delta
   # The y position changes depending on time passed
	#position.y = initial_y + amplitude * sin(frequency * time_passed)
	move_and_slide()
...
func shoot_feather():
	# Instantiate the slime
	var feather = feather_scene.instantiate() as RigidBody2D
	feather.position = global_position 
	# Calculate direction towards the player
	var direction = Vector2.LEFT if randi() % 2 == 0 else Vector2.RIGHT
	feather.direction = direction 

	# Add slime to the scene
